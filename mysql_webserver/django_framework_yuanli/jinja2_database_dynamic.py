...
def f3(request):
    ...
    conn = MySQLdb.connect(host="127.0.0.1",
                           ...
                           passwd="Nmamtf@013",
                           db="guanxi")
    cs = conn.cursor(cursorclass=MySQLdb.cursors.DictCursor)

    cs.execute("select id,name,passwd from users")
    user_list = cs.fetchall()
    cs.close()
    conn.close()
    print(user_list)
    ...
    for row in user_list:
        tp = "<tr><td>%s</td><td>%s</td><td>%s</td></tr>" % (row["id"],
                                                             row["name"],
                                                             row["passwd"])
        content_list.append(tp)
    content = "".join(content_list)

    f = open("userlist.html", 'r')
    tmp = f.read()
    f.close()

    data = tmp.replace("@@fdsafdf@@",content)
    return data
...
def f4(request):
    ...
    tmp = Template(data)
    data = tmp.render(xxxxx=user_list)
